...
public class Ball : MonoBehaviour
{
    ...
    [SerializeField] private GameObject powerUpPrefab, powerUpSizePrefab, powerDownPrefab;
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        if (collision.gameObject.CompareTag("block")) // Si colisionamos con un bloque
        {
            ...
            else if (random >= 0.2f && random < 1.35f)
            {
                Instantiate(powerUpSizePrefab, collision.transform.position, Quaternion.identity);
            }            
        } else 
        {
